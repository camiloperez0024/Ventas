...
namespace Ventas.Controllers
{
    ...
    public class CountriesController : Controller
    {
        ...
        public async Task<IActionResult> DetailsState(int? id)
        {
            ...
            State state = await _context.States.Include(s => s.Country).Include(s => s.Cities).FirstOrDefaultAsync(m => m.Id == id);
            if (state == null)
            {
                return NotFound();
            }
        ...
        public async Task<IActionResult> Create(Country country)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    _context.Add(country); // agrega el pais
                    await _context.SaveChangesAsync(); // para que si no lo llena bien al volver a la pagina le salga lleno el campo
                    return RedirectToAction(nameof(Index)); // devuelve a la vista de index
                }
        ...
        public async Task<IActionResult> AddState(StateViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    _context.Add(state); // agrega el pais
                    await _context.SaveChangesAsync(); // para que si no lo llena bien al volver a la pagina le salga lleno el campo
                    return RedirectToAction(nameof(Details), new { Id = model.CountryId }); // devuelve a la vista de index
                }
                catch (DbUpdateException dbUpdateException)
                {
                    if (dbUpdateException.InnerException.Message.Contains("duplicate"))
        ...
        public async Task<IActionResult> AddCity(CityViewModel model)
        {
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    _context.Add(city); // agrega el pais
                    await _context.SaveChangesAsync(); // para que si no lo llena bien al volver a la pagina le salga lleno el campo
                    return RedirectToAction(nameof(DetailsState), new { Id = model.StateId }); // devuelve a la vista de index
                }
                catch (DbUpdateException dbUpdateException)
                {
                    if (dbUpdateException.InnerException.Message.Contains("duplicate"))
        ...
        public async Task<IActionResult> Edit(int? id)
        {
            ...
            Country country = await _context.Countries.Include(c => c.States)
                .FirstOrDefaultAsync(c => c.Id == id); // manda una lista de estados vacia que es diferente a una null
            if (country == null)
            {
                return NotFound();
            }
